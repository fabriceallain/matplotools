...
""".format(plot=args.plot_type, x=args.x, y=args.y, col=args.col, hue=args.hue,
    ...
    if args.plot_type == "gridbar":
        ...
        grid = sns.FacetGrid(data, col=args.col, hue=args.hue, xlim=xlim,
                             size=5, sharex=config["sharex"], row=args.row,
                             sharey=config["sharey"])
        grid.map(sns.violinplot, args.x, args.y, scale="area")
    elif args.plot_type == "gridhist":
        # Compare number of observations for each distribution, if uneven =>
        # normed = True (See seaborn hist)
        grid = sns.FacetGrid(data, col=args.col, xlim=xlim, size=5,
        ...
        grid = sns.FacetGrid(data, col=args.col, size=4, row=args.row,
                             hue=args.hue)
        grid.map(plt.scatter, x=args.x, y=args.y)
    elif args.plot_type == "gridkde":
        logger.info("Drawing KDE plot on grid")
        grid = sns_facetgrid(data, args.x, config, kind="kde", y=args.y,
                             hue=args.hue, col=args.col, row=args.row,
                             xlim=args.xlim,
                             palette=config.get("palette", "muted"))
    ...
    if issubclass(grid.__class__, sns.Grid):
        ...
        if config["xrot"] and isinstance(grid, np.ndarray) \
                and len(grid.axes) > 1 and args.xrot:
    ...
    if hasattr(grid, 'savefig'):
        grid.savefig(args.output, dpi=200)
    else:
